...
class Data:
    ...
    # either to use DictReader and DictWriter depends largely on list_values input
    def edit_file(self, list_values, object='me'):
        lst = [str(i) for i in list_values]
    ...
    # output values: all, keys, all_values, specific_name_oriented_values
    def read_file(self, read_content, object=''):
        with open(self.file_name+'.csv', 'r', newline='') as file:
            ...
            for line in read_file:
                if read_content == 'values':
                    # the fourth value of the line: name(system default)
                    if line[3].lower() in object.lower():
                        self.data_buffer = []
                        return line
